...
public class Board {
	...
	private Cell[] original; // A copy of the board when it is cut up but not moved or rotated
	 ...
	 *
	 * @return - true or false if the board is finished
	 */
	public boolean isFinished() {

		// Check for any null cells and return false if the null cell doesn't match the
		// original
		int counter = 0;
		for (Cell c : cells) {
			if (c == null) {
				...
				// Compare the hashcode of the arrays to check for correct orientation
				if (Arrays.hashCode(c.getImage()) != Arrays.hashCode(original[counter].getImage())) {
					return false;
				}
			}
			counter++;
